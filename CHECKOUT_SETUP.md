...
1. In the Supabase SQL Editor for project `gphkitnnjdqbxgwqtjpl`, run `supabase/commerce.sql` after `schema.sql` and `registration.sql`, then run `supabase/catalog-management.sql`. This upgrades an earlier Razorpay-only `shop_orders` table for PayU and gives the contact admin control of catalogue images, prices, tags and stock. The catalogue seed only inserts missing products; it does not overwrite admin edits.
...
3. The initial stock of 10 for each product is provisional; enter actual counts before selling. Checkout rejects products with missing or insufficient stock. Add a reliable stock reservation, decrement and cancellation/expiry process before enabling real payments; a quantity check alone cannot prevent simultaneous orders from overselling. Then test sign-in, saved basket and wishlist, unavailable products, invalid addresses, price changes, payment failure, payment success, order history, and the admin **Mark fulfilled** action. Confirm that each order total matches the server catalogue subtotal plus ₹99. Decide tax treatment and confirm product contents before accepting live orders.
