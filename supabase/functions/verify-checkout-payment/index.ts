...
Deno.serve(async req => {
  ...
  try {
    ...
    const { data: order, error } = await db.from('shop_orders').select('id,user_id,total_paise,status,payment_provider,razorpay_order_id').eq('id', order_id).eq('user_id', user.id).single();
    if (error || !order) return json({ error: 'Order not found' }, 404);
    if (order.payment_provider !== 'razorpay' || !order.razorpay_order_id) return json({ error: 'Wrong payment provider' }, 400);
